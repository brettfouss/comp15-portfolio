...
// 3/5/2014
//
// This file contains the implementation of a Day class.
// Includes: constructors, add event function, insert day
//           function, and various functions to convert
//           times and dates to strings in order to 
//           export the calendar as a CSV. 
//
// Note: Main must schedule events through the add events
...
// addEvents() takes a CSV of events in the following format, and adds them to a calendar:
// 2/25/2014,2/25/2014,1100,1115,Event A
// 2/25/2014,2/25/2014,1200,1230,Event B

// The addEvents() method needs to create a Day if one does not exist for a particular
...
void Calendar::addEvents(string csvEvents) {
        ...
        while (getline(eventStream,eventLine)) {
                ...
                if (tempDay->scheduleEarliest(evt) == false) { // could not schedule the event!
                        cout << "Could not schedule the event: " << evt.eventCSV() << endl;
                }
        }
}

// getDay() returns the index in dayList of
// startDate. If the Day does not exist,
// the function returns NULL
Day *Calendar::getDay(date startDate) {

    for (int n = 0; n < dayCount; n++) {
	if ((dayList[n].getDate().day == startDate.day)     &&
	    (dayList[n].getDate().month == startDate.month) &&
	    (dayList[n].getDate().year == startDate.year))
	    return &dayList[n];
    }
    return NULL;
}
...
// This function returns a pointer to the Day so other functions can insert events
// into that Day.
Day *Calendar::insertDay(date startDate) {

    Day newDay = Day(startDate);

    if (dayList == NULL) {
	dayList = new Day[INIT_CAPACITY];
	dayCapacity = INIT_CAPACITY;
    } else if (dayCount == dayCapacity) 
	expandList();
    ...
    for (int n = 0; n < dayCount; n++) {
	if (newDay.isBefore(dayList[n])) {
	    for (int i = dayCount; i > n; i--) //shift
    ...
    dayCount++;
    return &dayList[dayCount-1];

}

// calendar prints the calendar day by day
// day printing is handled by day implementation file
