...
// 3/5/2014
//
// This file contains functions to test the functionality
// of the calendar. 
// Both days and events may be scheduled through main. 
//
// While events can be created through main, to be scheduled
// ON a day, the add events function of the calendar
// implementation should be used. Currently, the schedule
// earliest function of the dayDynamicArray implementation
// is used. 
//

#define TEST_convert24HourTime 0 
#define TEST_printDay 0
#define TEST_nextFreeBlock 1
#define TEST_importA 0
#define TEST_importB 0
#define TEST_importC 0
...
int main()
{
	...
	for (int n = 0; n <= 23; n++) {
		...
		int time = ((n*100) + i);
		cout << "Time: " << time << " ";
		cout << Event::convert24HourTime(time) << endl;
	    }
	}
        ...
        Event e4(930, 1230,"Event 4");
        Event e5(1100,2100,"Event 5");

	int x = 1;
	int y = 2;

	Event test(x, y, "event");

	for (int n = 0; n < 100; n++) {
	    d.scheduleEarliest(test);
	    //x *= 2;
	    //y *= 3;
	    test.setStartTime(x);
	    test.setEndTime(y);
	...
	d.cancelEvent(test);
        ...
        cout << "Next free block, event 5: " << d.nextFreeBlock(e5) << endl;
